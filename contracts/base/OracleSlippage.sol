...
import '@yumyumswap/contract-periphery/contracts/base/PeripheryImmutableState.sol';
import '@yumyumswap/contract-periphery/contracts/base/BlockTimestamp.sol';
import '@yumyumswap/contract-periphery/contracts/libraries/Path.sol';
import '@yumyumswap/contract-periphery/contracts/libraries/PoolAddress.sol';
import '@yumyumswap/contract-core/contracts/interfaces/IYumyumSwapPool.sol';
import '@yumyumswap/contract-periphery/contracts/libraries/OracleLibrary.sol';
...
abstract contract OracleSlippage is IOracleSlippage, PeripheryImmutableState, BlockTimestamp {
    ...
    function getBlockStartingAndCurrentTick(IYumyumSwapPool pool)
        internal
        view
    ...
    function getPoolAddress(
        ...
        address tokenB,
        uint24 fee
    ) internal view virtual returns (IYumyumSwapPool pool) {
        pool = IYumyumSwapPool(PoolAddress.computeAddress(factory, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }
        ...
        returns (int256 syntheticAverageTick, int256 syntheticCurrentTick)
    {
        ...
        for (uint256 i = 0; i < numPools; i++) {
            ...
            IYumyumSwapPool pool = getPoolAddress(tokenIn, tokenOut, fee);
            ...
            if (secondsAgo == 0) {
                ...
                (, currentTick, , , , , ) = IYumyumSwapPool(pool).slot0();
            }
