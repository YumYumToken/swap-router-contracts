...
contract OracleSlippageTest is OracleSlippage {
    mapping(address => mapping(address => mapping(uint24 => IYumyumSwapPool))) private pools;
    ...
    function getPoolAddress(
        ...
        address tokenB,
        uint24 fee
    ) internal view override returns (IYumyumSwapPool pool) {
        pool = pools[tokenA][tokenB][fee];
    }

    function testGetBlockStartingAndCurrentTick(IYumyumSwapPool pool)
        external
        view
